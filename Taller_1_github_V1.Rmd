...
$$
prop_{cgmt} = \alpha+\beta_1 Corrupt_{mt}+\beta_2Auditada_{mt}+Clavedelaescuela_{c}+year_{t}+grade_{gmt}+X_{mt}+u_{cgmt}  \quad (1)
$$
...
-  **Corrupt{mt}:** Esta es una variable dicotómica que toma el valor de uno si hubo corrupción en el municipio (m) en el año (t). El nivel de agregación es municipio y año.
...
- **X_{mt}:** Este es un vector de variables de control con nivel de agregacion municipio y año, que incluye:
  ...
  - **HOMI_CAP_MUN_{mt}:** El número de homicidios per cápita a nivel de municipio-año.
  - **log total_{mt}:** El logaritmo del total de impuestos recolectados en el municipio (m) en el año (t).
...
getwd()
setwd('/Users/jorgeviafara/Documents/Talleres_eva_impacto/Taller_1')

#Inspeccionar el directorio de trabajo
list.files()
list.dirs()

#Leer la base de datos corruption.dta
corruption_0 <- read_dta("corruption_SV.dta")

#Visualizar los datos de corruption
#glimpse(corruption_0)
#summary(corruption_0)

#Seleccion de columnas
corruption_1 <- corruption_0 %>%
...
$$
prop_{cgmt} = \alpha+\beta_1 Corrupt_{mt}+u_{cgmt}  \quad (2)
$$
```{r}
...
$$
prop_{cgmt} = \alpha+\beta_1 Corrupt_{mt}+\beta_2Auditada_{mt}+grade_{gmt}+PartidoDesf_{mt}+AlreadyAudited_{mt}+CorruptPast_{mt}+HOMICAPMUN_{mt}+MismoPartidoG_{mt}+Log(total)_{mt}+u_{cgmt}  \quad (3)
$$

```{r}
#Regresion del modelo simple de la variable dependiente contra la independiente principal + Controles.

modelo_b <- lm(prop ~ Corrupt + Auditada + GradoSecundaria + PartidoDesf + AlreadyAudited + CorruptPast + HOMI_CAP_MUN + log_total + MismoPartidoG, 
               data = corruption_clean)
...
$$
prop_{cgmt} = \alpha+\beta_1 Corrupt_{mt}+\beta_2Auditada_{mt}+Clavedelaescuela_{c}+year_{t}+u_{cgmt}  \quad (4)
$$
...
#Una regresión simple de la variable dependiente contra la independiente principal + Controles + Efectos fijos.
modelo_d <- plm(prop ~ Corrupt + Auditada + GradoSecundaria + PartidoDesf + AlreadyAudited + CorruptPast + HOMI_CAP_MUN + log_total + MismoPartidoG, 
...
stargazer(modelo_a, modelo_b, modelo_c, modelo_d,
          ...
          #keep = c("Corrupt"),  # Solo mostrar el coeficiente de interés
          omit = c("Auditada", "GradoSecundaria", "PartidoDesf", "AlreadyAudited", 
                   "CorruptPast", "HOMI_CAP_MUN", "log_total", "MismoPartidoG", "Constant"),
